...
#ifndef SLAM_DIAPACKAGE_ORBEXTRACTOR_H
#define SLAM_DIAPACKAGE_ORBEXTRACTOR_H
...
public:
     ...
     */
    ORBextractor(int _nfeatures,        //指定要提取的特征点数目
     ...
     */
    void extract_features(cv::InputArray _image, cv::InputArray _mask, std::vector<cv::KeyPoint>& _keypoints,cv::OutputArray _descriptors){
...
protected:
    ...
    std::vector<cv::KeyPoint> DistributeOctTree(
            const std::vector<cv::KeyPoint> &vToDistributeKeys, const int &minX, const int &maxX, const int &minY,
            const int &maxY,
            const int &nFeatures, const int &level)
    {
        ...
        while(!bFinish)
        {
            ...
            //2、当前所有的节点中都只包含一个特征点
            if((int)lNodes.size()>=nFeatures 				//判断是否超过了要求的特征点数
...
#endif //SLAM_DIAPACKAGE_ORBEXTRACTOR_H
